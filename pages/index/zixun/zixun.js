const app = getApp();

Page({
  // 新增本地知识库
  localKnowledgeBase: {
    // 1. 快乐情绪
    "happy": [
      ["开心|高兴|快乐|愉快|幸福|哈哈哈", 
       "😊 检测到积极情绪！保持好心情可以：\n\n" +
       "✨ 快乐放大法：\n" +
       "1. 记录此刻感受（建议写日记）\n" +
       "2. 与亲友分享快乐（快乐会传染）\n" +
       "3. 做些创造性活动（绘画/写作等）"],
       
      ["笑|哈哈|嘿嘿|嘻嘻", 
       "🤗 笑容是最好的情绪调节剂！\n\n" +
       "💡 小建议：\n" +
       "• 看喜剧片/搞笑视频\n" +
       "• 回忆开心往事\n" +
       "• 做鬼脸自拍"]
    ],
  
    // 2. 压力/焦虑
    "stress": [
      ["压力大|压力好大|喘不过气|压得慌", 
       "🧘 压力管理三步法：\n\n" +
       "1. 暂停呼吸法（吸气4秒→屏息4秒→呼气6秒）\n" +
       "2. 压力源分类法（区分可控/不可控因素）\n" +
       "3. 微小行动法（从最小可行动作开始）"],
       
      ["焦虑|心慌|坐立不安", 
       "🌿 焦虑缓解方案：\n\n" +
       "• 5-4-3-2-1 grounding 技术：\n" +
       "  找出5个可见物体→4种可触摸材质→3种环境声→2种气味→1种味道\n" +
       "• 渐进式肌肉放松（从脚趾到头皮逐个部位收紧→放松）"]
    ],
  
    // 3. 抑郁/低落
    "depression": [
      ["抑郁|低落|郁闷|没意思|活着没意思", 
       "🌧 您现在的感受很重要，建议：\n\n" +
       "1. 联系可信赖的人面对面交谈\n" +
       "2. 进行最低强度运动（如散步10分钟）\n" +
       "3. 预约专业心理咨询师\n\n" +
       "☎ 心理援助热线：\n" +
       "北京24小时：010-82951332\n" +
       "上海心理热线：021-12320-5"],
       
      ["想哭|难受|好难过", 
       "🤍 情绪释放是健康的：\n\n" +
       "• 准备安全环境（舒适空间+纸巾+温水）\n" +
       "• 设置计时器（允许自己哭5-10分钟）\n" +
       "• 哭后补充糖分（巧克力/香蕉等）"]
    ],
  
    // 4. 愤怒/烦躁
    "anger": [
      ["生气|愤怒|气死|火大|烦死了", 
       "🔥 愤怒能量转换法：\n\n" +
       "1. 物理释放：\n" +
       "   • 撕废纸\n" +
       "   • 捶打枕头\n" +
       "   • 快跑30秒\n" +
       "2. 认知重构：\n" +
       "   • 自问「这事3年后还重要吗？」"],
       
      ["烦躁|易怒|看什么都不顺眼", 
       "🌪 情绪降温技巧：\n\n" +
       "• 降温刺激：冷水洗脸/含冰块\n" +
       "• 嗅觉调节：闻薄荷精油\n" +
       "• 延迟反应：数到10再说话"]
    ],
  
    // 5. 孤独/寂寞
    "lonely": [
      ["孤独|孤单|没人理解|独来独往", 
       "🌌 孤独感转化指南：\n\n" +
       "1. 线上社群：\n" +
       "   • 豆瓣小组（搜索「同好」）\n" +
       "   • 心理类播客留言区\n" +
       "2. 线下接触：\n" +
       "   • 参加读书会/兴趣班\n" +
       "   • 定期宠物互动"],
       
      ["没朋友|社交恐惧|不会交际", 
       "🫂 社交分步练习：\n\n" +
       "• 阶段1：每周微笑问候3个陌生人\n" +
       "• 阶段2：参与1次线上讨论\n" +
       "• 阶段3：尝试15分钟线下交流"]
    ],
  
    // 6. 睡眠问题
    "sleep": [
      ["失眠|睡不着|熬夜|睡不好", 
       "🌙 睡眠改善方案：\n\n" +
       "1. 环境调整：\n" +
       "   • 使用遮光窗帘\n" +
       "   • 保持18-22℃室温\n" +
       "2. 行为训练：\n" +
       "   • 床只用于睡眠（不玩手机）\n" +
       "   • 固定起床时间（包括周末）"],
       
      ["多梦|噩梦|睡不踏实", 
       "🛌 梦境管理技巧：\n\n" +
       "• 睡前1小时避免刺激内容\n" +
       "• 噩梦后立即开灯+改变体位\n" +
       "• 练习「梦境改写」：\n" +
       "  白天想象噩梦变喜剧结局"]
    ],
  
    // 7. 恋爱关系
    "relationship": [
      ["分手|失恋|被甩", 
       "💔 失恋恢复阶段指南：\n\n" +
       "第1周：允许崩溃（设置发泄日）\n" +
       "第2周：物理隔离（删除联系方式）\n" +
       "第3周：重建生活（开发新习惯）"],
       
      ["吵架|冷战|感情变淡", 
       "❤️‍🩹 关系修复步骤：\n\n" +
       "1. 情绪记录表：\n" +
       "   记录冲突时的身体反应（如心跳加速）\n" +
       "2. 非暴力沟通：\n" +
       "   说「当...时，我感到...，我需要...」"]
    ],
  
    // 8. 工作/学业
    "work": [
      ["不想上班|工作没意思|职业倦怠", 
       "💼 职业倦怠应对：\n\n" +
       "• 微小成就感：\n" +
       "  每天记录3件完成的小事\n" +
       "• 工作意义重构：\n" +
       "  列出工作带来的间接价值"],
       
      ["拖延症|不想学习|效率低", 
       "📚 5分钟启动法：\n\n" +
       "1. 设置5分钟倒计时\n" +
       "2. 只做最简单部分（如打开文档）\n" +
       "3. 完成后立即奖励（如小块巧克力）"]
    ],
  
    // 9. 自我认知
    "self": [
      ["自卑|觉得自己没用|不如别人", 
       "🌟 优势发现练习：\n\n" +
       "1. 回忆过去3次被称赞经历\n" +
       "2. 完成VIA性格优势测试（免费在线版）\n" +
       "3. 建立「优点日记」每周记录"],
       
      ["迷茫|不知道要什么|没有目标", 
       "🧭 人生罗盘探索：\n\n" +
       "• 10年想象法：\n" +
       "  写「我希望10年后的普通一天」\n" +
       "• 反向排除法：\n" +
       "  列出绝对不想要的生活状态"]
    ],
  
    // 10. 家庭关系
    "family": [
      ["父母不理解|父母根本不理解|代沟|原生家庭", 
       "👨‍👩‍👧 代际沟通技巧：\n\n" +
       "1. 历史背景理解：\n" +
       "   了解父母成长时期的价值观\n" +
       "2. 非对抗表达：\n" +
       "   用「我们」代替「你」开头"],
       
      ["亲戚催婚|家庭压力", 
       "🛡 应对催婚策略：\n\n" +
       "• 标准应答模板：\n" +
       "  「正在努力中，有进展会告诉大家」\n" +
       "• 话题转移技巧：\n" +
       "  主动询问对方健康/旅游计划"]
    ],
  
    // 11. 躯体反应
    "physical": [
      ["心慌|手抖|出汗|身体不舒服", 
       "🩺 情绪躯体化应对：\n\n" +
       "1. 区分检查：\n" +
       "   • 先排除生理疾病（体检）\n" +
       "   • 记录发作情境（情绪日记）\n" +
       "2. 缓解技巧：\n" +
       "   • 双手浸入温水40秒"],
       
      ["没胃口|暴食|饮食失调", 
       "🍽 情绪化饮食管理：\n\n" +
       "• 饥饿感量表（1-10分评估）\n" +
       "• 替代行为清单：\n" +
       "  情绪激动时先做列表其他事（如拼图）"]
    ],
  
    // 12. 特殊情境
    "special": [
      ["疫情|隔离|封控", 
       "😷 隔离期心理维护：\n\n" +
       "• 建立生活锚点：\n" +
       "  固定时间做固定事（如晨间咖啡）\n" +
       "• 有限信息接触：\n" +
       "  每天只看2次权威疫情通报"],
       
      ["失业|被裁员|找工作难", 
       "💼 职业过渡期指南：\n\n" +
       "1. 经济规划：\n" +
       "   • 计算生存资金底线\n" +
       "2. 技能提升：\n" +
       "   • 每天2小时在线学习\n" +
       "3. 人脉激活：\n" +
       "   • 每周联系3个前同事"]
    ],
  
    // 默认回复（当无匹配时）
    "default": "正在为您深度分析情绪...\n\n💡 您可以先尝试：\n• 深呼吸三次\n• 喝半杯温水\n• 记录此刻感受"
  },

  data: {
    messages: [],
    inputValue: '',
    isSending: false,
    scrollTop: 0,
    userAvatar: '',
    apiError: false
  },

  onLoad: function() {
    this.getUserAvatar();
    this.addWelcomeMessage();
  },

  onShow: function() {
    this.getUserAvatar();
  },

  getUserAvatar: function() {
    const userInfo = wx.getStorageSync('userInfo');
    if (userInfo?.avatarUrl) {
      this.setData({ userAvatar: userInfo.avatarUrl });
    }
  },

  onInput: function(e) {
    this.setData({ inputValue: e.detail.value });
  },

  // 修改后的发送逻辑
  sendMessage: function() {
    const content = this.data.inputValue.trim();
    if (!content || this.data.isSending) return;

    this.addUserMessage(content);
    this.setData({ inputValue: '', isSending: true });

    // 先尝试本地匹配
    const localReply = this.matchLocalKnowledge(content);
    if (localReply !== this.localKnowledgeBase.default) {
      setTimeout(() => {
        this.addBotMessage(localReply, true);
        this.setData({ isSending: false });
      }, 600); // 模拟思考时间
    } else {
      // 复杂问题调用API
      this.callDeepseekApi(content);
    }
  },

  // 新增：本地知识匹配
  matchLocalKnowledge: function(text) {
    const kb = this.localKnowledgeBase;
    text = text.toLowerCase();
    
    // 优先级匹配
    for (const [keyword, rules] of Object.entries(kb)) {
      if (keyword === "default") continue;
      
      for (const [pattern, reply] of rules) {
        const regex = new RegExp(pattern, "i");
        if (regex.test(text)) {
          return reply;
        }
      }
    }
    return kb.default;
  },

  // 修改后的API调用
  callDeepseekApi: function(content) {
    const thinkingId = this.addThinkingMessage();

    wx.cloud.callFunction({
      name: 'deepseekchat',
      data: { text: content },
      success: res => {
        this.removeThinkingMessage(thinkingId);
        if (res.result?.reply) {
          this.addBotMessage(res.result.reply, true);
        } else {
          this.showAPIError("分析失败，请重试");
        }
      },
      fail: err => {
        this.removeThinkingMessage(thinkingId);
        console.error("API调用失败:", err);
        this.showAPIError("服务暂时不可用");
        
        // API失败时尝试返回本地兜底回答
        const fallbackReply = this.getFallbackReply(content);
        this.addBotMessage(fallbackReply, true);
      },
      complete: () => {
        this.setData({ isSending: false });
      }
    });
  },

  // 新增：API失败时的兜底回复
  getFallbackReply: function(text) {
    const kb = this.localKnowledgeBase;
    for (const [keyword, rules] of Object.entries(kb)) {
      if (keyword === "default") continue;
      for (const [pattern] of rules) {
        if (new RegExp(pattern, "i").test(text)) {
          return "网络不稳定，但检测到您可能需要：\n" + 
                 rules.find(r => r[0] === pattern)[1];
        }
      }
    }
    return "网络连接异常，请稍后再试\n您可以先试试：\n1. 深呼吸放松\n2. 记录当前感受";
  },

  // 以下保持原有消息管理方法不变
  addWelcomeMessage: function() {
    this.addMessage({
      role: 'assistant',
      content: '您好，我是情感分析助手。简单问题我会快速回复，复杂问题将深度分析。',
      time: this.formatTime(new Date()),
      id: Date.now()
    });
  },

  addUserMessage: function(content) {
    this.addMessage({
      role: 'user',
      content: content,
      time: this.formatTime(new Date()),
      id: Date.now()
    });
  },

  addBotMessage: function(content, isAnalysis = false) {
    this.addMessage({
      role: 'assistant',
      content: content,
      time: this.formatTime(new Date()),
      id: Date.now(),
      isAnalysisResult: isAnalysis
    });
  },

  addThinkingMessage: function() {
    const id = 'thinking_' + Date.now();
    this.addMessage({
      role: 'assistant',
      content: '正在分析中...',
      time: this.formatTime(new Date()),
      id: id,
      isThinking: true
    });
    return id;
  },

  removeThinkingMessage: function(id) {
    this.setData({
      messages: this.data.messages.filter(msg => msg.id !== id)
    });
  },

  addMessage: function(message) {
    this.setData({
      messages: [...this.data.messages, message]
    }, this.scrollToBottom);
  },

  scrollToBottom: function() {
    this.setData({ scrollTop: 999999 });
  },

  showAPIError: function(message) {
    this.addMessage({
      role: 'assistant',
      content: `⚠️ ${message}`,
      time: this.formatTime(new Date()),
      id: Date.now()
    });
    this.setData({ apiError: true });
  },

  formatTime: function(date) {
    return date.toTimeString().substr(0, 5);
  }
});